...
class ListViewController: UITableViewController {
  ...
  func fetchPhotoDetails() {
    ...
    NSURLConnection.sendAsynchronousRequest(request, queue: OperationQueue.main) {response,data,error in
      if data != nil {
        let datasourceDictionary: NSDictionary = try! PropertyListSerialization.propertyList(from: data!, options: [], format: nil) as! NSDictionary//PropertyListSerialization.propertyListWithData(data, options: Int(PropertyListSerialization.MutabilityOptions.Immutable.rawValue), format: nil, error: nil) as! NSDictionary
        
        for data in datasourceDictionary {
          let name = data.key as? String
          let url = URL(string:data.value as? String ?? "")
          if name != nil && url != nil {
